...
//             XRaiseWindow(dpy, ev.xkey.subwindow);
         ...
         */
            ...
            printf("CreateNotify: %d, parent: %d\n", (int)w, (int)ev.xcreatewindow.parent);
            Client::createClient(w, dpy);
        } else if(ev.type == DestroyNotify) {
            Window w = ev.xdestroywindow.window;
            printf("DestroyNotify: %d\n", (int)w);
            Client::windowDestroyed(w);
        } else if(ev.type == MapNotify) {
            Client::mapNotify(ev.xmap.window);
        } else if(ev.type == UnmapNotify) {
            Client::unmapNotify(ev.xunmap.window);
        } else {
//             printf("event type: %d\n",ev.type);
//             fflush(stdout);
        }
...
int main()
{
    ...
    XChangeWindowAttributes(dpy, root, CWEventMask, &new_root_attr);
    ...
    XQueryTree(dpy, root, &unused, &unused, &children, &num_children);

    for (unsigned int i = 0; i < num_children; i++) //FIXME - what if a window gets destroyed inbetween ?
        Client::createClient(children[i], dpy);
