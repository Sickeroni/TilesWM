...
void X11Client::refreshSizeHints()
{
    ...
    if (XGetWMNormalHints(dpy(), _widget->wid(), &size_hints, &supplied_fields)) {
        ...
        if (supplied_fields & PMaxSize) {
            _size_hints.max_width = size_hints.max_width;
            _size_hints.max_height = size_hints.max_height;
        }
        if (supplied_fields & PMinSize) {
            _size_hints.min_width = size_hints.min_width;
            _size_hints.min_height = size_hints.min_height;
        }
    } else
